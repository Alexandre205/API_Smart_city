...
export const validateDemandeCreate = async(req,res,next) =>{
    try {
        req.val = await demandeValidator.creationValidator.validate(req.body);
        next();
    }catch(error){
        console.log(error);
...
export const validateDemandeUpdate = async(req,res,next) =>{
    try {
        req.val = await demandeValidator.updateValidator.validate(req.body);
        next();
    }catch(error){
        console.log(error);
        res.status(400).json({ errors: error.messages });
...
export const validateDemandeDelete = async(req,res,next) =>{
    try {
        req.val = await demandeValidator.deleteValidator.validate(req.params);
        next();
    }catch(error){
        console.log(error);
        res.status(400).json({ errors: error.messages });
