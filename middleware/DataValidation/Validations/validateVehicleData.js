...
export const validateVehiculeCreate = async(req,res,next) =>{
    try {
        req.val = await vehiculeValidator.creationValidator.validate(req.body);
        next();
    }catch(error){
...
export const validateVehicleUpdate = async(req,res,next) =>{
    try {
        req.val = await vehiculeValidator.updateValidator.validate(req.body);
        next();
    }catch(error){
        res.status(400).json({ errors: error.messages });
    }
...
export const validateVehiculeDelete = async(req,res,next) =>{
    try {
        req.val = await vehiculeValidator.deleteValidator.validate(req.params);
        next();
    }catch(error){
        res.status(400).json({ errors: error.messages });
    }
