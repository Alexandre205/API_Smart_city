...
export const validateTrajetCreate = async(req,res,next) =>{
    try {
        req.val = await trajetValidator.creationValidator.validate(req.body);
        next();
    }catch(error){
...
export const validateTrajetUpdate = async(req,res,next) =>{
    try {
        req.val = await trajetValidator.updateValidator.validate(req.body);
        next();
    }catch(error){
        res.status(400).json({ errors: error.messages });
    }
...
export const validateTrajetDelete = async(req,res,next) =>{
    try {
        req.val = await trajetValidator.deleteValidator.validate(req.params);
        next();
    }catch(error){
        res.status(400).json({ errors: error.messages });
    }
